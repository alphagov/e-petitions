...
RSpec.describe Archived::GovernmentResponse, type: :model do
  ...
  describe "callbacks" do
    context "when the government response is created" do
      let(:petition) { FactoryBot.create(:archived_petition, :awaiting) }
    ...
    context "when the government response is deleted" do
      ...
      context "and the petition has more than 10,000 signatures" do
        let(:petition) { FactoryBot.create(:archived_petition, :response, government_response_at: 1.week.ago, response_threshold_reached_at: 6.months.ago) }
        ...
        it "updates the government_response_at timestamp" do
          expect {
            ...
            petition.reload.government_response_at
          }.from(be_within(1.second).of(1.week.ago)).to(nil)
        end
      ...
      context "and the petition has less than 10,000 signatures" do
        let(:petition) { FactoryBot.create(:archived_petition, :response, government_response_at: 1.week.ago, response_threshold_reached_at: nil) }

...
RSpec.describe "invalid encoding", type: :request do
  context "when sponsoring a petition" do
  ...
  context "when validating a sponsor signature" do
  ...
  context "when validating a signature" do
  ...
  context "when unsubscribing a signature" do
    let(:petition) { FactoryBot.create(:open_petition) }
  ...
  context "when unsubscribing an archived signature" do

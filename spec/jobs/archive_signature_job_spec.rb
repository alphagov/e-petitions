...
RSpec.describe ArchiveSignatureJob, type: :job do
  ...
  let(:signature_ids) { petition.signatures.pluck(:id) }

  it "copies every signature" do
    expect {
      described_class.perform_now(petition, archived_petition, signature_ids)
    }.to change {

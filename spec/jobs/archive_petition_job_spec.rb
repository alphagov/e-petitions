...
RSpec.describe ArchivePetitionJob, type: :job do
  ...
  context "with a petition that has a debate scheduled" do
    let(:petition) do
      FactoryBot.create(:closed_petition, debate_scheduled_at: 2.days.ago, debate_scheduled_on: 3.days.ago, scheduled_debate_date: 2.weeks.from_now)
    end

    it "copies the attributes" do
      expect(archived_petition.debate_scheduled_at).to eq(petition.debate_scheduled_at)
      expect(archived_petition.debate_scheduled_on).to eq(petition.debate_scheduled_on)

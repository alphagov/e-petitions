class AddModeratedByToPetitions < ActiveRecord::Migration[5.2]
  ...
  def up
    ...
    unless column_exists?(:archived_petitions, :moderated_by_id)
      add_column :archived_petitions, :moderated_by_id, :integer
      add_index :archived_petitions, :moderated_by_id, algorithm: :concurrently
  ...
  def down
    ...
    if column_exists?(:archived_petitions, :moderated_by_id)
      remove_foreign_key :archived_petitions, column: :moderated_by_id
      remove_index :archived_petitions, :moderated_by_id
      remove_column :archived_petitions, :moderated_by_id
    end
  end
end

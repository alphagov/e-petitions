class AddLowerIndexToSignaturesEmail < ActiveRecord::Migration[6.1]
  ...
  def up
    ...
    unless index_exists?(:index_archived_signatures_on_lower_email)
      execute <<~SQL
        CREATE INDEX CONCURRENTLY index_archived_signatures_on_lower_email
        ON archived_signatures USING btree ((lower(email)));
      SQL
    end
  end
  ...
  def down
    ...
    if index_exists?(:index_archived_signatures_on_lower_email)
      execute <<~SQL
        DROP INDEX CONCURRENTLY index_archived_signatures_on_lower_email;
      SQL
    end
  end

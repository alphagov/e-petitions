class AddIndexesOnSignatureCountAndCreatedAtToPetitions < ActiveRecord::Migration[6.1]
  ...
  def up
    ...
    unless index_exists?(:index_archived_petitions_on_signature_count_and_created_at)
      execute <<~SQL
        CREATE INDEX CONCURRENTLY index_archived_petitions_on_signature_count_and_created_at
        ON archived_petitions (signature_count DESC, created_at DESC);
      SQL
    end
  end
  ...
  def down
    ...
    if index_exists?(:index_archived_petitions_on_signature_count_and_created_at)
      execute <<~SQL
        DROP INDEX CONCURRENTLY index_archived_petitions_on_signature_count_and_created_at;
      SQL
    end
  end

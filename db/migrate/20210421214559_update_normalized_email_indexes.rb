class UpdateNormalizedEmailIndexes < ActiveRecord::Migration[6.1]
  ...
  def up
    ...
    unless index_exists?(:index_archived_signatures_on_lower_normalized_email)
      execute <<~SQL
        CREATE INDEX CONCURRENTLY index_archived_signatures_on_lower_normalized_email
    ...
    if index_exists?(:index_archived_signatures_on_normalized_email)
      execute <<~SQL
        DROP INDEX CONCURRENTLY index_archived_signatures_on_normalized_email
      SQL
    end
  end
  ...
  def down
    ...
    unless index_exists?(:index_archived_signatures_on_normalized_email)
      execute <<~SQL
    ...
    if index_exists?(:index_archived_signatures_on_lower_normalized_email)
      execute <<~SQL
        DROP INDEX CONCURRENTLY index_archived_signatures_on_lower_normalized_email
      SQL
    end
  end

module AdminHubHelper
  ...
  def recently_in_moderation_count
    Petition.with_no_tags.in_moderation.between_in_moderation_times(from: Site.moderation_near_overdue_in_days.days.ago, to: Time.current).count
  end

  def nearly_overdue_moderation_count
    Petition.with_no_tags.in_moderation.between_in_moderation_times(from: Site.moderation_overdue_in_days.days.ago, to: Site.moderation_near_overdue_in_days.days.ago).count
  end

  def overdue_moderation_count
    Petition.with_no_tags.in_moderation.overdue_in_moderation_time_limit.count
  end

  def tagged_count
    Petition.with_any_tags.in_moderation.count
  end

module SocialMetaHelper
  def open_graph_tag(name, content, interpolation = {})
    if Symbol === content
      tag(:meta, property: "og:#{name}", content: I18n.t(content, **(interpolation.merge(scope: :'metadata.open_graph'))))
    elsif name == 'image'
  ...
  def twitter_card_tag(name, content, interpolation = {})
    if Symbol === content
      tag(:meta, name: "twitter:#{name}", content: I18n.t(content, **(interpolation.merge(scope: :'metadata.twitter'))))
    elsif name == 'image'

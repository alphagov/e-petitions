module ArchivedPetitionHelper
  ...
  def petition_duration_to_words(duration)
    ...
    if duration.frac.zero?
      pluralize(duration.floor, "month", "months")
    elsif duration.frac > 0.75
      "nearly #{pluralize(duration.ceil, "month", "months")}"
    elsif duration.frac < 0.25
      "just over #{pluralize(duration.floor, "month", "months")}"
    else
      "over #{pluralize(duration.floor, "month", "months")}"
    end
  end
end

class Admin::EmailTemplatesController < Admin::AdminController
  ...
  def create
    if @template.save
      redirect_to_index_url notice: :email_template_created
    else
      respond_to do |format|
  ...
  def update
    if @template.update(template_params)
      redirect_to_index_url notice: :email_template_updated
    else
      respond_to do |format|
  ...
  def destroy
    redirect_to_index_url notice: :email_template_deleted
  end

  def activate
    if @template.activate
      redirect_to_index_url notice: :email_template_activated
    else
      redirect_to_index_url notice: :email_template_not_activated
    end
  end

  def deactivate
    if @template.deactivate
      redirect_to_index_url notice: :email_template_deactivated
    else
      redirect_to_index_url notice: :email_template_not_deactivated
    end
  end
